...
def kepco_scraper(request):
    request_json = request.get_json(silent=True)

    print("Scraping:")
    df = ks.get_kepco_as_dataframe()
    print(" - Got Data")
    print("Converting:")
...
def _upload_blob_to_storage(df):
    ...
    BLOB_NAME = 'kepco_historical_data.csv'

    """Uploads a file to the bucket."""
    bucket = CS.get_bucket(BUCKET_NAME)
    blob = bucket.blob(BLOB_NAME)

    blob.upload_from_string(ks.convert_kepco_df_to_csv(df))
